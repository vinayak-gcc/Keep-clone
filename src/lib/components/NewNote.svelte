...
<!-- New Note Form -->
<div class="mb-8 border  space-y-2 p-0 dark:bg-gray-800 dark:text-white rounded-lg max-w-[30rem] w-full md:w-[30rem] lg:w-[40rem] place-content-center mx-auto transition-colors duration-300"
  ...
  <div class="flex flex-col px-2 group dark:bg-[#202124] dark:text-white dark:border-white dark:border">
    <div class="flex gap-2 w-full">
      <div class="w-full mt-1">
        <input type="text" bind:value={newTitle} placeholder="Title" class="w-full border-none outline-none shadow-none bg-transparent dark:placeholder-white placeholder-black focus:ring-white dark:focus:ring-[#202124] "  />
      </div>
      <div>
        <button on:click={() => (newPinned = !newPinned)} class={`p-1 rounded-full mt-2.5 hover:bg-gray-100 ${newPinned ? 'text-yellow-500' : 'text-gray-400'}`} aria-label="Pin note" title="Pin or Unpin Note">
    ...
    <div class="hidden group-visited:block group-focus-within:block">
      <div>
        <input bind:value={newContent} placeholder="Take a note..." class="p-2 w-full border-none shadow-none bg-transparent dark:placeholder-white placeholder-black focus:ring-white dark:focus:ring-[#202124] " />
      </div>
      <div class="flex items-center justify-between py-2">
        <div class="flex gap-2 relative">
          ...
          <button on:click={handleAddNote} class="px-4 py-2 w-fit h-fit bg-blue-500 text-white rounded-md hover:bg-blue-600">
            Add Note
          </button>
        </div>
      </div>
