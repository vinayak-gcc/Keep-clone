<script lang="ts">
  import { unpinnedNotes, gridLayout} from '../../Store/store';
        ...
        <div class="relative z-10 h-full flex flex-col">
          <div class="flex items-center justify-between mb-2">
            <input
              ...
              on:input={(e) => {
                ...
                if (target) updateNote(id, target.value, content , userEmail );
              }}
            />
            <div class="flex gap-2">
              <button on:click={() => togglePin(id, pinned , userEmail )} class="text-yellow-500">
                {pinned ? '📌' : '📍'}
              </button>
            </div>
          </div>
          ...
          <textarea
            ...
            on:input={(e) => {
              ...
              if (target) updateNote(id, title, target.value , userEmail );
            }}
          ></textarea>
